...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    // Customer email submit function below
    $email_customer = EMAIL_TEXT_HEADER . EMAIL_TEXT_FROM . STORE_NAME . "\n\n" .
	    $this->customer['firstname'] . ' ' . $this->customer['lastname'] . "\n\n" .
   	 "Thank you for your order. Advance Tec will contact you when we have received the order and send you an invoice." . "\n\n" . EMAIL_DETAILS_FOLLOW . "\n" .
	    EMAIL_SEPARATOR . "\n" .
   	 EMAIL_TEXT_ORDER_NUMBER . ' ' . $zf_insert_id . "\n" .
	    EMAIL_TEXT_DATE_ORDERED . ' ' . strftime(DATE_FORMAT_LONG) . "\n" .
   	 EMAIL_TEXT_INVOICE_URL . ' ' . zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $zf_insert_id, 'SSL', false) . "\n\n";
    
    zen_mail($this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_customer, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout', $this->attachArray);
