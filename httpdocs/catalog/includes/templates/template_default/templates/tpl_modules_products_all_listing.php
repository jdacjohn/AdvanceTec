...
// more info in place of buy now
      if (PRODUCT_ALL_BUY_NOW != '0' and zen_get_products_allow_add_to_cart($products_all->fields['products_id']) == 'Y') {
        if (zen_has_product_attributes($products_all->fields['products_id'])) {
         //$link = '<a class="moreInfoLink" href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
        } else {
...
//            $how_many++;
        ...
        $display_products_button = zen_get_buy_now_button($products_all->fields['products_id'], $the_button, $products_link) . '<br />' . zen_get_products_quantity_min_units_display($products_all->fields['']) . str_repeat('<br clear="all" />', substr(PRODUCT_ALL_BUY_NOW, 3, 1));
      } else {
      ...
      if (PRODUCT_ALL_LIST_DESCRIPTION > '0') {
        ...
        $display_products_description = stripslashes(zen_trunc_string($disp_text, PRODUCT_ALL_LIST_DESCRIPTION, '<!--<br/> --> <a class="moreInfoLink" href="' . zen_href_link(zen_get_info_page($products_all->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($products_all->fields['master_categories_id']) . '&products_id=' . $products_all->fields['products_id']) . '"> ' . MORE_INFO_TEXT . '</a>'));
      } else {
        $display_products_description = '';
      }
              ...
              <?php
                ...
                while (!$disp_sort_order->EOF) {
                  ...
                  if ($disp_sort_order->fields['configuration_key'] == 'PRODUCT_ALL_BUY_NOW') {
                    if($display_products_name <> '<a href="http://advancetec.uppercasedesigngroup.com/catalog/index.php?main_page=product_info&amp;cPath=2&amp;products_id=6"><h3 class="itemTitle2">F01 Safe Deposit Box Lease Agreement</h3></a>'){
            ...
            <td colspan="2" valign="top" class="main">
              <?php
                $disp_sort_order = $db->Execute("select configuration_key, configuration_value from " . TABLE_CONFIGURATION . " where configuration_group_id='" . $group_id . "' and (configuration_value >= 2000 and configuration_value <= 2999) order by LPAD(configuration_value,11,0)");
					...
					if($products_all->fields['products_id'] == 39){
						$width = "width:500px;";
					}elseif($products_all->fields['products_id'] == 41){
						$width = "width:500px;";
					}elseif($products_all->fields['products_id'] == 10){
						$width = "width:500px;";
					}else{
						$width = "width:460px;";
					}
                while (!$disp_sort_order->EOF) {
                  ...
                  if ($disp_sort_order->fields['configuration_key'] == 'PRODUCT_ALL_LIST_NAME') {
                    echo "<div style='padding-left:10px;'>".$display_products_name."</div>";
                  ...
                  if ($disp_sort_order->fields['configuration_key'] == 'PRODUCT_ALL_LIST_PRICE') {
                    echo "<div style='padding-left:10px;".$width."'>".$display_products_price;
                    echo $display_products_description."</div>";
              ...
              <?php
              //echo $products_all->fields['master_categories_id']."cpath";
              //echo "<br />";
              //echo $products_all->fields['products_id']."products";

					include '../catalog/includes/languages/english/product_info.php';
					include '../catalog/includes/templates/template_default/templates/main_template_vars_all_products.php';
?>
            </td>
          </tr>
