...
<br class="clearBoth" />
<h2 id="checkoutPaymentHeadingAddress"><?php echo "Billing Information:"; ?></h2>
...
<div class="floatingBox important forward"><?php echo "Your billing address is shown to the left. You can change the billing address by clicking the Change Address button."; ?></div><br clear="all">
<?php
  if (zen_count_shipping_modules() > 0) {
?>
<h2 id="checkoutShippingHeadingMethod" style="visibility:hidden"><?php echo TABLE_HEADING_SHIPPING_METHOD; ?></h2>

<?php
    if (sizeof($quotes) > 1 && sizeof($quotes[0]) > 1) {
?>

<div id="checkoutShippingContentChoose" class="important" style="visibility:hidden"><?php echo TEXT_CHOOSE_SHIPPING_METHOD; ?></div>

<?php
    } elseif ($free_shipping == false) {
?>
<div id="checkoutShippingContentChoose" class="important" style="visibility:hidden"><?php echo TEXT_ENTER_SHIPPING_INFORMATION; ?></div>
...
?>
<!---<fieldset class="fieldBgColor" id="shippingdiv" style="visibility:hidden">--->
<fieldset class="fieldBgColor" id="shippingdiv" style="background-color:#fff">
<h3><?php echo $quotes[$i]['module']; ?>&nbsp;<?php if (isset($quotes[$i]['icon']) && zen_not_null($quotes[$i]['icon'])) { echo $quotes[$i]['icon']; } ?></h3>
...
<?php
            if ( ($n > 1) || ($n2 > 1) ) {
?>
<div class="important forward" style="visibility:hidden"><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], (isset($quotes[$i]['tax']) ? $quotes[$i]['tax'] : 0))); ?></div>
<?php
            } else {
?>
<div class="important forward" style="visibility:hidden"><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])) . zen_draw_hidden_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id']); ?></div>
<?php
            }
?>
